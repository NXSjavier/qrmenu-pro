    ...
    <div className="min-h-screen flex items-center justify-center p-4">
      <div className="w-full max-w-md mx-auto">
        <div className="text-center mb-6 sm:mb-8">
          ...
          <h1 className="text-xl sm:text-2xl md:text-3xl font-extrabold text-white">Crear Restaurante</h1>
          <p className="text-gray-500 text-xs sm:text-sm mt-1">Tu menú digital en minutos</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="backdrop-blur-xl border rounded-2xl sm:rounded-3xl p-6 sm:p-8 space-y-4 sm:space-y-5" style={{ background: 'rgba(255,255,255,0.03)', borderColor: 'rgba(255,215,0,0.1)', backdropFilter: 'blur(40px)' }}>
          ...
          <div>
            <label className="block text-gray-400 text-xs sm:text-sm font-semibold mb-1.5 sm:mb-2">Nombre del restaurante</label>
            <input
              type="text"
              value={nombre}
          ...
          <p className="text-center text-gray-500 text-xs sm:text-sm">
            ¿Ya tienes cuenta?{' '}
            <a href="/admin/login" className="font-semibold" style={{ color: '#FFD700' }}>Iniciar sesión</a>
          </p>
        </form>
      </div>
    </div>
